...
def plot_forecast(test_actuals_inv, test_preds_inv, date_index, output_dir, n_outputs = 0):
    """
    ...
    Args:
        ...
        date_index (pd.DatetimeIndex): Dates corresponding to each sample.
        n_outputs (int): Forecast horizon (number of predicted steps).
        output_dir (str): Directory where the plot will be saved.
    """
    plt.figure(figsize=(14, 5))

    if n_outputs == 1:
        plt.plot(date_index, test_actuals_inv.flatten(),
                 label='Actual Cash Flow', linestyle='-', linewidth=1.5, color="#196194")
        plt.plot(date_index, test_preds_inv.flatten(),
                 label='Predicted Cash Flow', linestyle='--', linewidth=1.5, color="#8d1a1a")
    else:
        # Plot 1-step ahead
        plt.plot(date_index, test_actuals_inv[:, 0],
                 label='Actual (t+1)', linestyle='-', linewidth=1.5, color="#196194")
        plt.plot(date_index, test_preds_inv[:, 0],
                 label='Predicted (1-Day Ahead)', linestyle='--', linewidth=1.5, color="#8d1a1a")

        # Plot n-step ahead (shifted to align visually)
        shifted_index = date_index + pd.Timedelta(days=n_outputs - 1)
        plt.plot(shifted_index, test_preds_inv[:, -1],
                 label=f'Predicted ({n_outputs}-Days Ahead)',
                 linestyle=':', linewidth=1.5, color="#d68b27", alpha=0.8)
        plt.plot(date_index, test_actuals_inv[:, -1],
